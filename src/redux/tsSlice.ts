...
import { ChartopEntry, ChartableEntry } from "../types/timeseries";
...
function getMetricFormattedValue(metric: Metric, value: number) {
  if (metric.unit === Units.SECONDS) {
    ...
    if (value > MAX_FLOAT) {
      ...
      return (
        (value > 0 ? "+" : "") + (value * 100).toFixed(2).toString() + " %"
      );
    }
  } else {
...
export function addFormattedValuesToChartableEntry(
  entry: ChartableEntry,
  metrics: Array<Metric>,
) {
  const uidToMetric = Object.fromEntries(
    metrics.map((metric) => [metric.uid, metric]),
  );
  for (const operand of entry.operands) {
    for (const tsToMetric of operand.metadata.metrics || []) {
      tsToMetric.formattedValue = getMetricFormattedValue(
        uidToMetric[tsToMetric.uid],
        tsToMetric.value,
      );
      tsToMetric.color = getMetricColor(
        uidToMetric[tsToMetric.uid],
        tsToMetric.value,
      );
    }
...
function addFormattedValues(
  entry: ChartopEntry,
  metrics: Array<Metric>,
  orderByMetric: Metric,
) {
  addFormattedValuesToChartableEntry(entry, metrics);
  entry.formattedValue = getMetricFormattedValue(
    orderByMetric,
    entry.order_by_metric_value,
  );
  entry.color = getMetricColor(orderByMetric, entry.order_by_metric_value);
}

export const fetchTS = createAsyncThunk(
  "ts/fetchTS",
    ...
    builder
      ...
      .addCase(fetchTS.fulfilled, (state, action) => {
        ...
        for (const [i, entry] of (
          action.payload?.data?.data?.chartop_entries || []
        ).entries()) {
          const cacheKey = page * perPageCount + i;
...
const selectChartopEntriesCache = (state: RootState) =>
  state.ts.chartopEntriesCache;
