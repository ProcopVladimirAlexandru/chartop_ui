  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(fetchTags.fulfilled, (_state, action) => {
        ...
        newState.result.data = newState.result.data.filter( (tag: Tag) => !isTagHidden(tag) ).sort(
          (tag_1: Tag, tag_2: Tag) => {
...
export const selectAllTags = (state: {tags: TagsState}) => state.tags.result?.data || [];

export const selectTagsStatus = (state: {tags: TagsState}) => state.tags.status;