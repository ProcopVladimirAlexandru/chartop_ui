...
function FilteredSearch() {
  ...
  const [selectedMetric, setSelectedMetric] = useState(
    stateFilters.metric || null,
  );
  const [ascOrDesc, setAscOrDesc] = useState(
    stateFilters.ascOrDesc || selectedMetric?.default_order || "desc",
  );
  ...
  function onDeleteTag(deletedTag: Tag) {
    const newSelectedTags = selectedTags.filter(
      (tag: Tag) => tag.uid != deletedTag.uid,
    );
    setSelectedTags(newSelectedTags);
  }
        ...
        onClick={() => {
          dispatch(
            setTSFilters({
              ...
              page: 0,
              perPageCount:
                stateFilters.perPageCount ||
                (width < 640 || height < 640 ? 3 : 3),
            }),
          );
