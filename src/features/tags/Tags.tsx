...
function Tags({
  ...
  const selectedTagsJSX = selectedTags.map((tag) => {
    return (
      <Grid key={tag.uid}>
        <Chip
  ...
  const availableTags = allAvailableTags.filter(
    (tag: Tag) =>
      !selectedTags.filter((selectedTag) => selectedTag.uid === tag.uid).length,
  );
  ...
  return (
    ...
    >
      {/*Title*/}
      <Typography level="h3">Choose the tags that interest you:</Typography>
      ...
      >
        <Grid xs={10}>
          <Autocomplete
            ...
            options={availableTags.sort((a: Tag, b: Tag) =>
              (a.category || "Others").localeCompare(b.category || "Others"),
            )}
            clearOnBlur={false}
            ...
            }} // TODO investigate TS error here
