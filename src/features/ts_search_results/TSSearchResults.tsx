...
import {
  selectFilteredChartopEntriesCache,
  selectTSStatus,
  selectTSResult,
} from "../../redux/tsSlice";
...
function TSSearchResults() {
  ...
  if (tsStatus === "error") {
    return (
      <AppError
        code={reduxResult?.code}
        message={reduxResult?.message}
      ></AppError>
    );
  ...
  if (!data || !Array.isArray(data.chartop_entries)) {
    return (
      <AppError message="Encountered unexpected data format. Something is wrong ..."></AppError>
    );
  }
  ...
  const paginationElement = (
    <TSPagination lastPage={tsElementsJSX?.length === 0}></TSPagination>
  );
  ...
  return (
    <Grid container={true} spacing={2} direction="column" ref={paginationRef}>
      <Grid>{paginationElement}</Grid>
      {tsElementsJSX}
      <Grid>{paginationElement}</Grid>
    </Grid>
  );
