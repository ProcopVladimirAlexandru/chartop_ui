...
import { ChartopEntry } from "../../types/timeseries";
...
function getChartDataFromServerData(entry: ChartopEntry) {
  ...
  if (entry.operands.length == 1) {
    for (let i = 0; i < entry.operands[0].timestamps.length; ++i) {
      chartData.push([
        new Date(entry.operands[0].timestamps[i]),
        entry.operands[0].values[i],
      ]);
    }
    return chartData;
  }
...
function GoogleTSChart({
  ...
  const smallScreen: boolean = isScreenSmall(windowWidth, windowHeight);
  ...
  const googleChartOptions = {
    ...
    title: smallScreen
      ? undefined
      : "Select area to zoom in. Right click to zoom out.",
    titlePosition: "in",
    titleTextStyle: {
    ...
    hAxis: {
      ...
      viewWindow: {
        min: smallScreen
          ? chartData[Math.max(chartData.length - 50, 1)][0]
          : null,
    ...
    explorer: {
      ...
      actions: smallScreen
        ? ["dragToPan", "rightClickToReset"]
        : ["dragToZoom", "rightClickToReset"],
    },
    curveType: "none",
    legend: {
      position: "bottom",
      textStyle: { color: theme.colorSchemes[mode].chart.legend },
    },
    chartArea: {
      left: smallScreen ? "20%" : "10%",
      top: "10%",
    ...
    backgroundColor: style.getPropertyValue(
      theme.colorSchemes[mode].palette.background.body,
    ),
  };
